...
var opt = document.getElementsByClassName('optbutton');
var forms = document.getElementById('forms');
var spanResult = document.getElementById('result');
var go = document.getElementById('go');
var loadbar = document.getElementById('loadbar');
var saveCheck = document.getElementById('keepcheck');
...
//function that define a type of easing
...
var main = function(){
  ...
  optselector();
	...
	$('form').submit(function(event){
		...
		if (randomize) { wordGen(); search = arrStr; }
    else search = $input.val();

		if (search != "") {
			callAjax(search);
			spanResult.innerHTML = "Recent uploads tagged " + search;
      $input.val("");
		}
...
//function that control the scrolling effect of the page when
//a tag is selected, fires the loading bar and submit
var smoothscroll = function(){
  $(function() {
    $('a.tag').bind('click',function(event){
        var $anchor = $(this);
        ...
        setTimeout(function() {
          $input.val(event.target.innerHTML.slice(1));
          opt[0].click();
          loadbar.style.display = 'block';
          loadbar.classList.add('loader');
          finishLoad();
        }, 1800);
    });
    });
...
setTimeout(function() {
    ...
    loadbar.classList.remove('loader');
}, 3200);
};

//function that move the image layer down to show images tags 
//by adding a listener to each image
var hoverimg = function(){
    var covers = document.getElementsByClassName('cover');
    for (var i = 0; i < covers.length; i++) {
      covers[i].addEventListener("mouseenter", function(e){
        e.target.lastElementChild.classList.add('layerdown');
        //setTimeout(function() {

        //}, 1000);
      },false);
      covers[i].addEventListener("mouseleave", function(e){
        e.target.lastElementChild.classList.remove('layerdown');
      },false);
    }
};

//function that highlight the selected option button
var optselector = function(){ // 
    for (var i = 0; i < opt.length; i++) {

      opt[i].addEventListener("click", function(e){
      //if (!mobileMode) searchElem.placeholder = "search pics by " + e.srcElement.name;
      if (e.srcElement.name === "random") {
        randomize = true;
        searchElem.setAttribute("disabled","disabled");
      }
      else {
        randomize = false;
        searchElem.removeAttribute("disabled");
      }
      // toggle the buttons background  
      optionNum = e.srcElement.id;
      $('#'+optionNum).css('background','#00f28f');
      
      if (optionNum === "option1") {
        $('#'+e.srcElement.nextElementSibling.id).css('background','#B0A2D2');
      }
      else if (optionNum === "option2") {
        $('#'+e.srcElement.previousElementSibling.id).css('background','#B0A2D2');
      }
...
//function that handle various page resize changes
var resizer = function(){
    ...
    function props() {
        if (window.innerWidth < 618 ) { //mobile
                                        mobileMode = true; desktopMode = false;
                                        searchElem.placeholder = "search...";
                                        $('.btn').css('padding','20px 14px');
                                        $('.main').css('margin-top','40px');
                                        $.colorbox.resize();
                                      }
        else { //desktop
               desktopMode = true; mobileMode = false;
               searchElem.placeholder = "search pics by...";
               $('.btn').css('padding','20px 24px');
...
var jsonFlickrFeed = function (data) {
    ...
    //var listDisplay = htmlString !== "" ? "inline-table" : "none" ;

    var htmlString = "";

    // Cycling through array of Flickr photos
    $.each(data.items, function(i,item){

        // Thumbnails fetching
        var sourceSquare = (item.media.m);//.replace("_m.jpg", "_s.jpg"); //http://farm2.staticflickr.com/1644/26739518785_ef363c32ce_m.jpg

        htmlString += '<li style="display:' + 'none' + ';"><div class="components"><div class="cover">';
        htmlString += ashtagIt(item.tags) + '<img class="layer" src="' + sourceSquare;
        htmlString += '" alt="'+ item.title +'" /></div><div class="text">'; 
        htmlString += '<a href="' + item.link + '" target="_blank"><span>' + shortenIt(item.title) + '</span></a>';
        htmlString += '<a class="author" href="https://www.flickr.com/photos/' + item.author_id + '" target="_blank">';
        htmlString += '<span>By ' + item.author.slice(20,-1) + '</span></a></div><span class="miniicon"></span><span class="miniopt">';
        htmlString += '<a class="gallery" rel="group" href="' + expandPic(sourceSquare) + '" target="_blank">show </a>|';
        htmlString += '<a id="remove"> remove</a></span></div></li>';

    });

    if (saveCheck.checked) {
      ul.insertAdjacentHTML('afterbegin',htmlString); // add the newly found list elements in the ul element before the old ones
      searchCount ++;
    }
    else {
      ul.innerHTML = htmlString;  // add the newly found list elements in the ul element
      searchCount = 1;
    }
    showItems();//show items progressively
     
    $('a.gallery').colorbox({transition:"elastic",returnFocus:false});

    $.colorbox.resize();

    var images = document.getElementsByClassName('layer');

    hoverimg();//add listener to images on hover

    openmini();//add listener to lower bar

    smoothscroll();//makes the scrolling smooth when selecting a tag
};


var openmini = function(){
  var arricons = document.getElementsByClassName('miniicon');
  for (var i = 0; i < arricons.length/searchCount; i++) {
      arricons[i].addEventListener('click', function(e){
        ...
        if (!miniVisible){
          e.target.nextSibling.classList.add('miniup');
          e.target.classList.add('arrowup');
          //console.log(e.target.nextSibling.lastElementChild);
          //e.target.css('transform','rotate(-135deg);');
        }
        else {
          e.target.nextSibling.classList.remove('miniup');
...
var expandPic = function(picadd){
  var result = picadd.slice(0,-6);
  result += '.jpg';
  return result;
};

//function that delays the images from being displayed
  ...
  for (var i=0; i < items.length; i++) {
      ...
      (function(i){
        ...
        window.setTimeout(function(){
         items[i].style.display = 'inline-table';
...
var ashtagIt = function (tags){
  var arr = tags.split(" ");
  var result = "";
  for (var s of arr) result = result + "<a href='#search' id='taglinks' class='tag'>" + "#" + s + " </a>";
  result.trim();
  return result;
};

var shortenIt = function (title){
  if (title.length > 73){
...
var callAjax = function (search){

  var searchTerm = search.split(" "); 
  search="";
  //console.log(searchTerm);
  for (var s of searchTerm) search = search + s + ",";
  search.slice(0,-1);

  var query = "http://api.flickr.com/services/feeds/photos_public.gne?format=json&tags="+search;
