...
var jsonFlickrFeed = function (data) {
    ...
    $.each(data.items, function(i,item){
        ...
        htmlString += '<li><div class="components"><div class="cover">'+ ashtagIt(item.tags) +'';
        htmlString += '<img class="layer" src="' + sourceSquare;
        htmlString += '" alt="'+ item.title +'" /></div><div class="text">'; 
        htmlString += '<a href="' + item.link + '" target="_blank"><span>' + shortenIt(item.title) + '</span></a>';
        htmlString += '<a href="https://www.flickr.com/photos/' + item.author_id + '" target="_blank">';
        htmlString += '<span>By ' + item.author.slice(20,-1) + '</span></a></div></div></li>';

    });

    // Pop HTML in the #item DIV
...
var ashtagIt = function (tags){
  var arr = tags.split(" ");
  var result = "";
  for (var s of arr) result = result + "#" + s + " ";
  result.trim();
  return result;
}
